...
// Sprite store.
static std::unordered_map<std::filesystem::path, std::shared_ptr<Sprite>> g_SpriteMap;
...
//std::shared_ptr<Sprite> ResourceManager::loadSprite(const std::filesystem::path& filePath)
//{
//    const auto iter = g_SpriteMap.find(filePath);
//
//    if (iter == g_SpriteMap.end())
//    {
//        auto sprite = std::make_shared<Sprite>(filePath);
//
//        g_SpriteMap[filePath] = sprite;
//
//        return sprite;
//    }
//
